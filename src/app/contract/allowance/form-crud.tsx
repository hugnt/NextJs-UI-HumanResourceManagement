...
import { Allowance, allowanceDefault, allowanceSchema } from "@/data/schema/allowance.schema";
...
export default function FormCRUD(props: FormProps) {
  ...
  const addDataMutation = useMutation({
    mutationFn: (body: Allowance) => allowanceApiRequest.create(body),
    onSuccess: () => {
  ...
  const updateDataMutation = useMutation({
    mutationFn: ({ id, body }: { id: number, body: Allowance }) => allowanceApiRequest.update(id, body),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: [QUERY_KEY.keyList] })
  ...
  const deleteDataMutation = useMutation({
    mutationFn: (id: number) => allowanceApiRequest.delete(id),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: [QUERY_KEY.keyList] })
  ...
  const form = useForm<Allowance>({
    ...
    defaultValues: allowanceDefault,
  });

  const onSubmit = (data: Allowance) => {
  ...
  const handleChangeName = (e: ChangeEvent<HTMLInputElement>) => {
    const updatedValue = "PARAM_ALLOWANCE_" + e.target.value.trim().normalize("NFD")
      .replace(/[\u0300-\u036f]/g, "")
      .replace(/đ/g, "d")
                ...
                <FormField control={form.control} name="name"
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Input placeholder="Enter allowance name" {...field} disabled={isDisabled} onChange={(e) => { field.onChange(e); handleChangeName(e); }} />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
                ...
                <FormField control={form.control} name="amount"
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Input placeholder="Enter allowance amount" {...field} disabled={isDisabled} />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
                ...
                <FormField control={form.control} name="terms"
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Input placeholder="Enter terms" {...field} disabled={isDisabled} />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
                ...
                <FormField control={form.control} name="parameterName"
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Input placeholder="PARAM_ALLOWANCE_" {...field} disabled />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
