...
export default function SignContract() {
    ...
    const handleSubmitData = () => {
        ...
        signContractMutation.mutate({ id: 1, body: formData });
            ...
            <Table className="border-collapse border">
                ...
                <TableBody>
                    {contractList.map((x, i) => (
                        <TableRow key={i}>
                            <TableCell className="font-medium">{"HD-" + x.id}</TableCell>
                            <TableCell>{x.name}</TableCell>
                            <TableCell>
                                <Badge className={`py-0 px-2 ${x.status ? "bg-green-500" : "bg-slate-500"}`}>{x.status ? "Chưa kí" : "Đã kí"}</Badge>
                            </TableCell>
                            <TableCell className="text-right space-x-1">
                                <Button size={"sm"} className="" onClick={() => handleOpenFormPDF(1)}>
                ...
                <AlertDialogContent className={`gap-0 border-none overflow-hidden p-0 sm:min-w-[500px] sm:max-w-[1000px] !sm:w-[1000px] sm:rounded-[0.3rem]`}>
                    ...
                    <Form {...form}>
                        {signMode == 1 ? <div className="flex w-full">
                            <iframe src={`https://localhost:7025/Contract/Nguyễn Đức Bình-hop-dong-so-1.pdf`} className="w-[65%] h-[600px]"></iframe>
                            <div className="w-[35%]">
                            ...
                            <div className="flex w-full">
                                <iframe src={`https://localhost:7025/Contract/Nguyễn Đức Bình-hop-dong-so-1_signed.pdf`} className="w-[100%] h-[600px]"></iframe>
                            </div>}
