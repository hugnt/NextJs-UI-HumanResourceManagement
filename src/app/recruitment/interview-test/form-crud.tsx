...
import { Test, testDefault, testSchema } from "@/data/schema/test.schema";
...
export default function FormCRUD(props: FormProps) {
  ...
  const addDataMutation = useMutation({
    mutationFn: (body: Test) => testApiRequest.create(body),
    onSuccess: () => {
  ...
  const updateDataMutation = useMutation({
    mutationFn: ({ id, body }: { id: number, body: Test }) => testApiRequest.update(id, body),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: [QUERY_KEY.keyList] })
  ...
  const deleteDataMutation = useMutation({
    mutationFn: (id: number) => testApiRequest.delete(id),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: [QUERY_KEY.keyList] })
                ...
                <FormField control={form.control} name="description"
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Input placeholder="Enter description" {...field} disabled={isDisabled} />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
