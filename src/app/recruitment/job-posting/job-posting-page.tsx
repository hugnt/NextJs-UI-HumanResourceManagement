...
export default function JobpostingPage() {
  ...
  const columnsDef: ColumnDef<JobPosting>[] = [
    {
      ...
      cell: ({ row }) => (
        <div className="w-[100px]">{row.getValue("positionName")}</div>
      ),
      enableSorting: false,
      enableHiding: false,
    },
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <div className="w-[100px]">{row.getValue("location")}</div>
      ),
      enableSorting: false,
      enableHiding: false,
    },
    {
      ...
      cell: ({ row }) => (
        <div className="w-[50px]">{row.getValue("salaryRangeMin")}</div>
      ),
      enableSorting: false,
      enableHiding: false,
    },
    {
      ...
      cell: ({ row }) => (
        <div className="w-[50px]">{row.getValue("salaryRangeMax")}</div>
      ),
      enableSorting: false,
      enableHiding: false,
    },
      ...
      cell: ({ row }) => {
        ...
        return (
          <div className="w-[100px]">
            {postingDate
              ? new Date(postingDate as string | number | Date).toLocaleString(
                  "en-GB",
                  {
                    day: "2-digit",
                    month: "2-digit",
      ...
      cell: ({ row }) => {
        ...
        return (
          <div className="w-[100px]">
            {expirationDate
              ? new Date(
                  expirationDate as string | number | Date
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <div className="w-[100px]">{row.getValue("experienceRequired")}</div>
      ),
      enableSorting: false,
      enableHiding: false,
    },
    {
      ...
      cell: ({ row }) => (
        <div className="w-[100px]">{row.getValue("employeeName")}</div>
      ),
      enableSorting: false,
      enableHiding: false,
    },
  ...
  const handlePosting = (row: Row<JobPosting>) => {
    const id = row.original.id;
    const selectedRecruitmentWeb: RecruitmentWeb = {
      jobPostingId: id,
      webId: 0,
    };
