...
export default function FormTest(props: FormTest) {
  ...
  const onSubmit = (data: TestResult) => {
    ...
    const totalPoints =
      formattedData?.reduce((acc, item) => acc + item.point, 0) || 0;
    console.log(data.applicantId);

    if (formattedData) {
      const fetchData = async () => {
        await testResultApiRequest.createList(formattedData);
        await applicantApiRequest.updatePoint(
          data.applicantId ?? 0,
          totalPoints
        );
      ...
      <Sheet open={openTest} onOpenChange={setOpenTest}>
        <SheetContent className="p-0 overflow-y-auto sm:max-w-[800px] !sm:w-[800px] min-w-[800px]">
          <SheetHeader className="px-4 pt-3">
            <SheetTitle>Chi tiết kiểm tra</SheetTitle>
          </SheetHeader>
          <Form {...form}>
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      >
                        ...
                        <SelectContent>
                          {listDataApplicant.data?.metadata?.map(
                            (item, index) => {
                              return (
                                <SelectItem
                                  key={index}
                        ...
                        <FormItem className="flex items-center justify-between space-x-3 w-full">
                          ...
                          <Input
                            ...
                            onChange={(e) => {
                              const newPoints = {
                                ...points,
                                [item.id ?? 0]: Number(e.target.value),
                              };
                              setPoints(newPoints);
                            }}
