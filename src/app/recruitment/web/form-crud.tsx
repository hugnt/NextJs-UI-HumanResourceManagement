...
import { Web, webDefault, webSchema } from "@/data/schema/web.schema";
...
export default function FormCRUD(props: FormProps) {
  ...
  const addDataMutation = useMutation({
    mutationFn: (body: Web) => webApiRequest.create(body),
    onSuccess: () => {
  ...
  const updateDataMutation = useMutation({
    mutationFn: ({ id, body }: { id: number, body: Web }) => webApiRequest.update(id, body),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: [QUERY_KEY.keyList] })
  ...
  const deleteDataMutation = useMutation({
    mutationFn: (id: number) => webApiRequest.delete(id),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: [QUERY_KEY.keyList] })
  ...
  const form = useForm<Web>({
    resolver: zodResolver(webSchema),
    defaultValues: webDefault,
  });

  const onSubmit = (data: Web) => {
          ...
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-0">
              <div className="p-2 text-sm space-y-3">
                ...
                />
                <FormField control={form.control} name="webApi"
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>WebApi</FormLabel>
                      <FormControl>
                        <Input placeholder="Enter full webApi" {...field} disabled={isDisabled} />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
